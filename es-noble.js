	...
	async function processDownload(saveAsZip) {
		...
		pauseResumeButton.onclick = () => {
			...
			if (isPaused) {
				...
				statusElement.textContent = "다운로드 일시정지됨 (재개 버튼을 눌러주세요)";
			} else {
				pauseResumeButton.textContent = "일시정지";
				pauseResumeButton.style.backgroundColor = "#ff9800";
				statusElement.textContent = "다운로드 재개됨...";
			}
		};
		...
		for (let i = startingIndex; i >= endingIndex; i--) {
			...
			if (pauseInterval > 0 && completedEpisodes > 0 && completedEpisodes % pauseInterval === 0) {
				...
				pauseResumeButton.textContent = "재개";
				pauseResumeButton.style.backgroundColor = "#4caf50";
				statusElement.textContent = `⚠️ ${pauseInterval}화 완료! 자동 일시정지됨 (재개 버튼을 눌러주세요)`;
			}
			...
			try {
				...
				console.error(`[downloadNovel] fetchNovelContent 오류: ${fetchError.message}`);
				result = null;
			}
...
function extractEpisodeLinks() {
	const episodeLinks = [];
	const links = document.querySelectorAll(".cell.td.cell_subj a");
...
async function runCrawler() {
	...
	const novelPageRule = "https://newtoki.live";
	...
	continueButton.onclick = async () => {
		...
		const pauseIntervalInput = createInputGroup(
			...
			"53",
			"일시정지 간격 입력",
			"💡 지정한 화수마다 자동으로 일시정지됩니다. 0으로 설정하면 자동 일시정지가 비활성화됩니다.",
			(value) => {
				if (Number.isNaN(Number(value)) || Number(value) < 0) {
					return "유효한 간격을 입력해주세요. (0 이상)";
				}
				return null;
			},
		);
		...
		downloadButton.onclick = () => {
			...
			console.log("[runCrawler] 일시정지 간격 입력:", pauseInterval);
			if (Number.isNaN(pauseInterval) || pauseInterval < 0) {
				alert("유효한 일시정지 간격을 입력해주세요. (0 이상)");
				console.log("[runCrawler] 잘못된 일시정지 간격 입력, 종료");
				return;
			}
			...
			console.log(
				`작업 추가됨: ${title} 다운로드 준비 중 (${startEpisode}화부터 ${endEpisode}화까지, 일시정지 간격: ${pauseInterval}화)`,
			);
